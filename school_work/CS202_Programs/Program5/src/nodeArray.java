...
public class nodeArray{
    ...
    protected node [] Array = new node[3];
    ...
    public String display(){
        ...
        for(int i = 0; i < 3; ++i){
            if( Array[i] != null ) {
                str.append('\n' + Names[i] + '\n');
                str.append(Array[i].displayNumbered());
            }
        }
        return str.toString();
    }
    ...
    public featureBase findWheels(int num){
        return Array[2].find(num);
    }

    public featureBase findWheels(String name){
        return Array[2].find(name);
    }

    public featureBase findEngine(int num){
        return Array[1].find(num);
    }

    public featureBase findEngine(String name){
        return Array[1].find(name);
    }

    public featureBase findColor(int num){
        return Array[0].find(num);
    }

    public featureBase findColor(String name){
        return Array[0].find(name);
    }

    private boolean copyArray(nodeArray Obj){
        if( Obj == null ) return false;
        for(int i = 0; i < 3; ++i)
            if( Obj.Array[i] != null )
                Array[i] = new node(Obj.Array[i]);
        return true;
    }
