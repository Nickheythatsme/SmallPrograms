...
//and min are.
unsigned int planet::random_value(unsigned int min, unsigned int max)
{
...
float planet::random_float(float min, float max)
{
    //Help make random more random by giving it the current clock time.
    srand( clock() );
...
bool planet::make_moon_name(int i, char *& moon_name)
{
